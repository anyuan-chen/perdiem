...
export default function MonthBreakdown({
  ...
  const lastMonthDeltaPercentage = Math.round(
    100 - (spent / spentTwoMonthsAgo) * 100
  );
  ...
  return (
    <div className="flex container cs-bg">
      ...
      <div className="flex-column p-20 w-auto" style={{ maxWidth: "1000px" }}>
        <h1 className="title">
          In January, you spent {formatter.format(spent)}
        </h1>
        ...
        <div className="flex gap-6 p-6 w-full h-80 flex-wrap">
          ...
          <div className="bar-container w-2/5 flex-grow flex flex-col justify-center items-center p-6 bg-white">
            ...
            <h4 className="title">{Math.abs(lastMonthDeltaPercentage)}%</h4>
            <h2 className="font-medium text-2xl text-gray-400">
              {lastMonthDeltaPercentage < 0 ? "less" : "more"} than last month
            </h2>
          </div>
        </div>
        <p className="font-medium text-lg text-gray-700">
          {username},{" "}
          {lastMonthDeltaPercentage > 0
            ? "you improved! But"
...
export const getServerSideProps = async (context) => {
  ...
  const lastMonth = dayjs().subtract(1, "month").format("YYYY");
  const lastLastMonth = dayjs().subtract(2, "month").format("YYYY");

  const spent = history
    .filter((i) => i.time.startsWith(lastMonth))
    .map((i) => i.amount)
    .reduce((a, b) => a + b, 0);

  const spentTwoMonthsAgo = history
    .filter((i) => i.time.startsWith(lastLastMonth))
    .map((i) => i.amount)
    .reduce((a, b) => a + b, 0);
  ...
  for (const c of [
    ...
    const total = history
      .filter((i) => i.category === c && i.time.startsWith(lastMonth))
      .map((i) => i.amount)
      .reduce((a, b) => a + b, 0);

    const totalTwoMonthsAgo = history
      .filter((i) => i.category === c && i.time.startsWith(lastLastMonth))
      .map((i) => i.amount)
      .reduce((a, b) => a + b, 0);
    ...
    categories.push({
      ...
      totalMoney: total || 0,
      changePercentage: (total / totalTwoMonthsAgo) * 100 || 0,
    });
  }
  ...
  for (const i of Object.values(shops)) {
    greatestIncome.push({
      ...
      category: i.category,
      total: history
        .filter((a) => a.name === i.name && a.time.startsWith(lastMonth))
        .map((a) => a.amount)
        .reduce((a, b) => a + b, 0),
    });
  }
  ...
  for (const i of history) {
    ...
    if (date.month() === +lastMonth && dayTotals[dateFormatted]) {
